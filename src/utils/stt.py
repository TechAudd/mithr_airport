...
def transcribe_audio(audio_bytes):
    with tempfile.NamedTemporaryFile(delete=False, suffix='.wav') as temp_file:
        ...
        result = whisper_model.transcribe(temp_file.name, language='en', fp16=False, verbose=False)
        os.unlink(temp_file.name)
        return result["text"].strip()

def voice_input(text, filename='output.wav', max_record_seconds=10, silence_duration=1.0):
    SAMPLE_RATE = 16000
    FRAME_SAMPLES = 512
    ...
    if audio_buffer:
        ...
        transcribed_text = transcribe_audio(full_audio.tobytes())
        print(f"{text} {transcribed_text}")
        wav_write(filename, SAMPLE_RATE, full_audio)
        return transcribed_text
        # print(f"Saved {len(full_audio)/SAMPLE_RATE:.2f}s recording to {filename}")
    else:
